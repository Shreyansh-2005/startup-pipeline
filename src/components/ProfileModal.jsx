...
export default function ProfileModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Dialog */}
      <div className="glass-modal relative w-full max-w-md overflow-hidden rounded-2xl bg-white dark:bg-zinc-950 p-6 shadow-2xl border border-zinc-200 dark:border-zinc-800 animate-in fade-in zoom-in-95 duration-200">
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 p-1.5 rounded-lg bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200 transition-all cursor-pointer"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="mb-6">
          <h2 className="text-xl font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2">
            <User className="w-5 h-5 text-indigo-455 dark:text-indigo-400" />
            My Outreach Profile
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Resume Upload (PDF) */}
          <div>
            <label className="block text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1.5 flex items-center justify-between animate-in fade-in duration-300">
          ...
          {/* Skills */}
          <div>
            <label className="block text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1.5 flex items-center justify-between">
              <span>Skills</span>
          ...
          <div className="flex gap-3 pt-3">
            ...
            <button
              type="submit"
              className="flex-1 bg-indigo-600 hover:bg-indigo-550 active:bg-indigo-700 text-white py-2 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/10 border border-indigo-500/20 hover:border-indigo-500/35 transition-all cursor-pointer"
            >
